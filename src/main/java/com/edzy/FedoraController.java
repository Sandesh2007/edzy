...
public class FedoraController {
    @FXML
    private TextArea textArea;

    private File selectedFile;
    private static String LOG_FILE_DIR;
    private static final String LOG_FILE_BASE_NAME = "error_log";
    ...
    void onSelectClick(ActionEvent event) {
        ...
        fileChooser.setTitle("Open Fedora Package");
        fileChooser.getExtensionFilters().add(new ExtensionFilter("Fedora Packages", "*.rpm"));
    ...
    void onInstallClick(ActionEvent event) {
        if (selectedFile != null) {
            try {
                ...
                String username = getUsername();
                if (username != null) {
                    // Construct the command with the full path to the .rpm file
                    String rpmFilePath = selectedFile.getPath();
                    String command = "pkexec sudo dnf install " + rpmFilePath;
                ...
                e.printStackTrace();
                String logFilePath = getUniqueLogFilePath();
                logException(e, logFilePath);
                textArea.setText("An error occurred while executing the command.\nLog File: " + logFilePath);
            }
        } else {
            textArea.setText("No file selected. Please select a Fedora package first.");
        }
    }
    ...
    private String getUniqueLogFilePath() {
        ensureLogDirectoryExists();
        String baseLogFilePath = LOG_FILE_DIR + "/" + LOG_FILE_BASE_NAME;
        String logFilePath = baseLogFilePath + ".txt";
        int counter = 1;
        while (new File(logFilePath).exists()) {
            logFilePath = baseLogFilePath + "_" + counter + ".txt";
            counter++;
        }
        return logFilePath;
    }
